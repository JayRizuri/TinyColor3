...
class TinyColor {
	constructor(color, opts) {
		...
		if (this.red < 1) this.red = Math.round(this.red);
		if (this.green < 1) this.green = Math.round(this.green);
		if (this.blue < 1) this.blue = Math.round(this.blue);

		this.isValid = rgb.ok;
		this._tc_id = tinyCounter++;

		this.brightness =
			(this.red * 299 + this.green * 587 + this.blue * 114) /
			1000;
		...
		this.rgbString =
			this.alpha === 1
				? `rgb(${Math.round(this.red)}, ${Math.round(
						this.green
				  )}, ${Math.round(this.blue)})`
				: `rgba(${Math.round(this.red)}, ${Math.round(
						this.green
				  )}, ${Math.round(this.blue)}, ${
						this.roundAlpha
				  })`;
	...
	brighten(amount) {
		...
		this.rgb.r = Math.max(
			...
			Math.min(
				255,
				this.rgb.r - Math.round(255 * -(amount / 100))
			)
		);
		this.rgb.g = Math.max(
			0,
			Math.min(
				255,
				this.rgb.g - Math.round(255 * -(amount / 100))
			)
		);
		this.rgb.b = Math.max(
			0,
			Math.min(
				255,
				this.rgb.b - Math.round(255 * -(amount / 100))
			)
		);
		return new TinyColor(this.rgb);
	}
	darken(amount) {
		amount = amount === 0 ? 0 : amount || 10;
		this.hsl.l -= amount / 100;
	...
	triad() {
		return [
			new TinyColor(this),
			new TinyColor({
				h: (this.hsl.h + 120) % 360,
				s: this.hsl.s,
				l: this.hsl.l
			}),
			new TinyColor({
				h: (this.hsl.h + 240) % 360,
				s: this.hsl.s,
				l: this.hsl.l
			})
		];
